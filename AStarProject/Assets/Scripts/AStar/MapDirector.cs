...
public class MapDirector : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (World.Active.GetExistingSystem<MapPainterSystem>() == null)
        {
            var PresentationSystemGroup = World.Active.GetExistingSystem<PresentationSystemGroup>();
            var mpSystem = World.Active.CreateSystem<MapPainterSystem>();
